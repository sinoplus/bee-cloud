  ...
  [SketchType.Type1]: {
    ...
    uniform float time;
		...
		void main()	{
		  ...
		  vec4 t1 = texture2D( textureFrom, (uv - 0.5) * (1.0 - intpl) + 0.5 ) ;
		  vec4 t2 = texture2D( textureTo, (uv - 0.5) * intpl + 0.5 );
  ...
  [SketchType.Type2]: {
    ...
    fragment: `
		...
		void main()	{
			...
			float border = 1.;
			vec2 newUV = (vUv - vec2(0.5))*resolution.zw + vec2(0.5);
			vec4 color1 = texture2D(textureFrom,newUV);
			vec4 color2 = texture2D(textureTo,newUV);
